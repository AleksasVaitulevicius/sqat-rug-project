...
import nl.tudelft.jpacman.npc.NPC; public class MapParser {
	 ...
	 */
	public MapParser(LevelFactory levelFactory, BoardFactory boardFactory) {Collect.Hit("MapParser.java","MapParser(LevelFactory levelFactory, BoardFactory boardFactory)");this.levelCreator = levelFactory; Collect.Hit("MapParser.java","MapParser(LevelFactory levelFactory, BoardFactory boardFactory)", "1094"); this.boardCreator = boardFactory; Collect.Hit("MapParser.java","MapParser(LevelFactory levelFactory, BoardFactory boardFactory)", "1131");}
	 ...
	 */
	public Level parseMap(char[][] map) {Collect.Hit("MapParser.java","parseMap(char[][] map)");int width = map.length; Collect.Hit("MapParser.java","parseMap(char[][] map)", "1774"); int height = map[0].length; Collect.Hit("MapParser.java","parseMap(char[][] map)", "1801"); Square[][] grid = new Square[width][height]; Collect.Hit("MapParser.java","parseMap(char[][] map)", "1834"); List<NPC> ghosts = new ArrayList<>(); Collect.Hit("MapParser.java","parseMap(char[][] map)", "1884"); List<Square> startPositions = new ArrayList<>(); Collect.Hit("MapParser.java","parseMap(char[][] map)", "1925"); makeGrid(map, width, height, grid, ghosts, startPositions); Collect.Hit("MapParser.java","parseMap(char[][] map)", "1979"); Board board = boardCreator.createBoard(grid); Collect.Hit("MapParser.java","parseMap(char[][] map)", "2046"); Collect.Hit("MapParser.java","parseMap(char[][] map)", "2095");return levelCreator.createLevel(board, ghosts, startPositions) ; }
	...
	private void makeGrid(char[][] map, int width, int height, Square[][] grid, List<NPC> ghosts, List<Square> startPositions) {Collect.Hit("MapParser.java","makeGrid(char[][] map, int width, int height, Square[][] grid, List<NPC> ghosts, List<Square> startPositions)");for (int x = 0; x < width; x++) {
		...
		} Collect.Hit("MapParser.java","makeGrid(char[][] map, int width, int height, Square[][] grid, List<NPC> ghosts, List<Square> startPositions)", "2295");}
	...
	private void addSquare(Square[][] grid, List<NPC> ghosts, List<Square> startPositions, int x, int y, char c) {Collect.Hit("MapParser.java","addSquare(Square[][] grid, List<NPC> ghosts, List<Square> startPositions, int x, int y, char c)");switch (c) {
		...
		default:
			throw new PacmanConfigurationException("Invalid character at "
					+ x + "," + y + ": " + c);
		} Collect.Hit("MapParser.java","addSquare(Square[][] grid, List<NPC> ghosts, List<Square> startPositions, int x, int y, char c)", "2581");}

	private Square makeGhostSquare(List<NPC> ghosts) {Collect.Hit("MapParser.java","makeGhostSquare(List<NPC> ghosts)");Square ghostSquare = boardCreator.createGround(); Collect.Hit("MapParser.java","makeGhostSquare(List<NPC> ghosts)", "3327"); NPC ghost = levelCreator.createGhost(); Collect.Hit("MapParser.java","makeGhostSquare(List<NPC> ghosts)", "3380"); ghosts.add(ghost); Collect.Hit("MapParser.java","makeGhostSquare(List<NPC> ghosts)", "3423"); ghost.occupy(ghostSquare); Collect.Hit("MapParser.java","makeGhostSquare(List<NPC> ghosts)", "3445"); Collect.Hit("MapParser.java","makeGhostSquare(List<NPC> ghosts)", "3475");return ghostSquare ; }
	 ...
	 */
	public Level parseMap(List<String> text) {Collect.Hit("MapParser.java","parseMap(List<String> text)");checkMapFormat(text); Collect.Hit("MapParser.java","parseMap(List<String> text)", "4030"); int height = text.size(); Collect.Hit("MapParser.java","parseMap(List<String> text)", "4057"); int width = text.get(0).length(); Collect.Hit("MapParser.java","parseMap(List<String> text)", "4086"); char[][] map = new char[width][height]; Collect.Hit("MapParser.java","parseMap(List<String> text)", "4125"); for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				map[x][y] = text.get(y).charAt(x);
			}
		} Collect.Hit("MapParser.java","parseMap(List<String> text)", "4168"); Collect.Hit("MapParser.java","parseMap(List<String> text)", "4295");return parseMap(map) ; }
	...
	private void checkMapFormat(List<String> text) {Collect.Hit("MapParser.java","checkMapFormat(List<String> text)");if (text == null) {
		...
		} Collect.Hit("MapParser.java","checkMapFormat(List<String> text)", "4543"); if (text.isEmpty()) {
			throw new PacmanConfigurationException(
					"Input text must consist of at least 1 row.");
		} Collect.Hit("MapParser.java","checkMapFormat(List<String> text)", "4654"); int width = text.get(0).length(); Collect.Hit("MapParser.java","checkMapFormat(List<String> text)", "4783"); if (width == 0) {
			throw new PacmanConfigurationException(
				"Input text lines cannot be empty.");
		} Collect.Hit("MapParser.java","checkMapFormat(List<String> text)", "4822"); for (String line : text) {
			if (line.length() != width) {
				throw new PacmanConfigurationException(
					"Input text lines are not of equal width.");
			}
		} Collect.Hit("MapParser.java","checkMapFormat(List<String> text)", "4937");}
	...
	public Level parseMap(InputStream source) throws IOException {Collect.Hit("MapParser.java","parseMap(InputStream source)");try (BufferedReader reader = new BufferedReader(new InputStreamReader(
		...
		} Collect.Hit("MapParser.java","parseMap(InputStream source)", "5539");}
}
